...
const OctileDistanceHeuristic = (a, b, D) => {
  ...
  const distance =
    D * Math.max(dx, dy) + (Math.sqrt(2) - 2 * D) * Math.min(dx, dy);
  return distance;
};
...
export const calcHeuristicDistance = (heuristic, heuristicWeight, a, b) => {
  switch (heuristic) {
    case "Manhattan":
      return (
        heuristicWeight * ManhattanDistanceHeuristic(a, b, heuristicWeight)
      );
    case "Euclidean":
      return (
        heuristicWeight * EuclideanDistanceHeuristic(a, b, heuristicWeight)
      );
    case "Chebyshev":
      return (
        heuristicWeight * ChebyshevDistanceHeuristic(a, b, heuristicWeight)
      );
    case "Octile":
