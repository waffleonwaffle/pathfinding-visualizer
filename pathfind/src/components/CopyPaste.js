...
const CopyPaste = ({
  ...
  const handleCopyGrid = async () => {
    const pathOnGrid = grid.find((row) =>
      row.find((cell) => cell.searched || cell.partOfPath)
    );
    if (pathOnGrid) {
      showErrorMessage("Clear the path first");
    }
    let inputtedGrid = grid
      .map((row) => row.map((cell) => JSON.stringify(cell)).join("\t"))
      .join("\n");
    const formattedBoardSettings =
      selectedAlgorithm +
      "}" +
      selectedHeuristic +
      "}" +
  ...
  const handlePasteGrid = async () => {
    ...
    try {
      ...
      const validObjectStrings = objectStrings
        .slice(5)
        .filter(Boolean)
        .map((objectString) => objectString.trim() + "}");
      const objects = validObjectStrings.map((objectString) =>
  ...
  return (
    <section className="grid-clipboard">
      <Button className="clipboard-button" onClick={handleCopyGrid}>
        Copy Grid
      </Button>
      <div className="error-message">{errorMessage}</div>
      <Button className="clipboard-button" onClick={handlePasteGrid}>
        Paste Grid
      </Button>
    </section>
  );
